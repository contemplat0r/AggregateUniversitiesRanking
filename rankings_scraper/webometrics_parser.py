...
def extract_data(html, description, absolute_ranking):
    ...
    for row in tree.xpath(table_row_xpath):
        ...
        if country_as_list != []:
            country = country_as_list[0].attrib.get('src').split('/')[-1].rstrip('.png').upper().encode('utf-8')
...
if __name__ == '__main__':
    ...
    table_row_xpath = '//table[@class="sticky-enabled"]/tbody/tr'
    ranking_value_xpath = 'td/center'
    university_name_xpath = 'td/a'
    country_xpath = 'td/center/img'
    ...
    for i in range(0, 121):
        path_string = 'WebometricsDownloadDir/Webometrics' + str(i) + '.html'
        # print path_string
    ...
    ranking_name_list = RankingName.objects.filter(short_name='Webometrics')
