...
MIN_TRUST_ABBR_LEN = 4
MIN_TRUST_LIST_LEN = 3
...
# For filling University and RankingValue tables must be call rawranking_records_to_structured_tables function or unkomment call to_database function in build_aggregate_ranking_dataframe function and call build_aggregate_ranking_dataframe 
...
def rawranking_records_to_dataframes(ranking_descriptions):
    ...
    for ranking_short_name, additional_processors in ranking_descriptions.items():
        ...
        #print 'rawranking_records_to_dataframes, columns: ', ranking_dataframe.columns
...
def reranked(grouped_by_rank_dict):
    ...
    for rank_value in sorted_aggregate_ranks:
        grouped_by_rank_dict[rank_value]['rank'] = i
        i = i + 1
...
def rawranking_records_to_structured_tables():
    this_function_start_time = timer()
    start = timer()
    dataframes_dict = rawranking_records_to_dataframes(ranking_descriptions)
    end = timer()
    print 'rawranking_records_to_structured_tables: rawranking_records_to_dataframes runtime = ', end - start

    start = timer()
    ranking_tables_dict = dataframes_to_ranking_tables(dataframes_dict)
    end = timer()
    print 'rawranking_records_to_structured_tables: dataframes_to_ranking_tables runtime = ', end - start
   
    start = timer()
    union_rank_tables = union_ranks(ranking_tables_dict)
    end = timer()
    print 'rawranking_records_to_structured_tables: union_ranks runtime = ', end - start
    ...
    print 'rawranking_records_to_structured_tables: to_database = ', end - start
   
    this_function_end_time = timer()
    print 'rawranking_records_to_structured_tables: total runtime = ', this_function_end_time - this_function_start_time
    return
...
if __name__ == '__main__':
    ...
    #names_list = [u'University of Trento', u'University of Canterbury', u'New York University', u'East China University of Science and Technology', 'Massachusetts Institute of Technology (MIT)', 'California Institute of Technology (Caltech)', 'UCL (University College London)', 'EPFL (Ecole Polytechnique Fédérale de Lausanne)', 'National Taiwan University of Science and Technology (Taiwan Tech)', 'Moscow State Institute of International Relations (MGIMO University)', 'Pontificia Universidade Católica de São Paulo (PUC -SP)', 'University of Milano- Bicocca']
